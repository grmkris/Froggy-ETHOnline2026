...
export type TurnBlock =
  | { readonly kind: "service"; readonly calls: readonly ToolCall[]; readonly index: number }
  | {
...
export const groupParts = (message: FroggyMessage): readonly TurnBlock[] => {
  ...
  const tasks = new Map<string, { readonly kind: "service"; readonly calls: ToolCall[]; readonly index: number }>();
  for (const block of blocks) {
    const result = block.kind === "tool" ? richResultOf(block.call) : null;
    if (block.kind !== "tool" || !result || !("service" in result)) { grouped.push(block); continue; }
    const previous = tasks.get(result.id);
    if (previous) { previous.calls.push(block.call); }
    else { const group = { kind: "service" as const, calls: [block.call], index: block.index }; tasks.set(result.id, group); grouped.push(group); }
  }
  return grouped;
};
