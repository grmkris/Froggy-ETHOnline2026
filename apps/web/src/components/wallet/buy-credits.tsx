...
import { CreditPurchase, HederaReceiving } from "@froggy/protocol";
import {
  Alert,
  AlertDescription,
  AlertTitle,
...
const creditFailure = (api: CreditsApi, error: Error | null): Error | null =>
  ...
  api.activity.error ??
  api.summary.error;
const CreditHederaReceiving = ({ api }: { readonly api: CreditsApi }) => {
  const receiving = useMutation({
    mutationFn: async () =>
      Schema.decodeUnknownSync(HederaReceiving)(
        await api.request("/hedera-account", { v: 1 })
      ),
    retry: false,
  });
  const detail = receiving.data;
  const address = detail?.accountId ?? detail?.alias;
  return (
    <div className="mb-4 flex flex-col gap-2">
      <Button
        variant="outline"
        disabled={receiving.isPending}
        onClick={() => {
          receiving.mutate();
        }}
      >
        {receiving.isPending
          ? "Preparing address…"
          : "Get HBAR receiving address"}
      </Button>
      {detail === undefined ? null : (
        <>
          <p className="text-muted-foreground text-xs">
            Send native HBAR on{" "}
            {detail.network === "hedera:mainnet"
              ? "Hedera mainnet"
              : "Hedera testnet"}{" "}
            to your address below. Return to review your credit purchase after
            the deposit confirms.
            {detail.stubbed ? " This is a simulated receiving address." : ""}
          </p>
          {address === null || address === undefined ? (
            <p role="alert">Your receiving address is not ready. Try again.</p>
          ) : (
            <Input
              aria-label="Your HBAR receiving address"
              readOnly
              value={address}
              onFocus={(event) => {
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-lg">
          ...
          <CreditWalletHelp wallet={wallet} api={api} />
        </DialogContent>
      </Dialog>
    </>
  );
