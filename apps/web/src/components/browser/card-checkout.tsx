...
const canReviewAgain = (checkout: CardCheckout, expired: boolean): boolean =>
  checkout.reconciledAt !== null ||
  (checkout.paymentDispatchedAt === null &&
    (checkout.approvedAt === null ||
      checkout.bridge === null ||
      checkout.bridge.destinationConfirmed) &&
    (expired || checkout.stage === "needs_help"));
  ...
  checkout.bridge === null ? null : (
    <div className="text-xs">
      ...
      {checkout.bridge.fillTransaction === null ? null : (
        <p className="font-mono break-all">{checkout.bridge.fillTransaction}</p>
      )}
    </div>
  );
const PurchaseReview = ({
  ...
  return (
    ...
    >
      ...
      {inspection !== null && inspection.paymentHosts.length > 0 ? (
        <p className="text-muted-foreground text-xs break-all">
          Payment frames: {inspection.paymentHosts.join(", ")}
        </p>
      ) : null}
      <BridgeProgress checkout={checkout} />
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {canReviewAgain(checkout, expired) ? (
          <Button
            variant="outline"
            disabled={busy}
            onClick={() => {
              act(async () => {
                await api.request(`/api/card-checkouts/${checkout.id}/review`, {
                  v: 1,
        ...
        checkout.reconciledAt === null ? (
          <Button
            ...
            onClick={() => {
              act(async () => {
                await api.request(
                  `/api/card-checkouts/${checkout.id}/reconcile`,
                  {
                    v: 1,
                    issuerReviewed: true,
                  }
          ...
          >
            I checked my issuer · release reservation
          </Button>
        ) : null}
...
export const BrowserCardCheckout = ({
  ...
  const checkouts = useCardCheckouts(methods.data?.enabled === true);
  const api = useCardApi();
