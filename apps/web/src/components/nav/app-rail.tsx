...
export const AppRail = ({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
        ...
        {recent.map((record) => (
          <Link
            ...
            aria-current={
              pathname === `/chat/${record.id}` ? "page" : undefined
            }
            className={cn(
              "text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:ring-ring flex min-h-11 items-center gap-2 rounded-xl px-3 text-xs outline-none focus-visible:ring-2",
              pathname === `/chat/${record.id}` && "bg-card text-foreground"
            )}
          >
      ...
      <nav
        className="mt-3 flex shrink-0 flex-col gap-0.5 border-t pt-3"
      ...
      >
        {SECONDARY_ITEMS.map((item) => (
          <Link
            ...
            className={cn(
              ROW,
              pathname === item.to && "bg-card text-foreground"
            )}
