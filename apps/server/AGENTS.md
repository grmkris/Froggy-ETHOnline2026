...
One Bun process: the SPA, the JSON API, both WebSockets, owner-authorized credit funding, the Telegram webhook, the schedule ticker, and one hosted browser per signed-in user. One origin, so there is no CORS configuration that exists only in development.
...
- `credit-funding.ts` — only the owner creates and pays immutable credit quotes. Base USDC and native HBAR settle through x402. Persist proof identity before settlement and signed EVM transaction bytes before broadcast. Recovery reuses the saved proof and credits the account once after chain confirmation.
- `packages/wallet/src/credit-store.ts` — the platform credit boundary: one account lock atomically creates a task and reserves its quoted units, then captures successful work or releases failed/cancelled work with the task result. Uncertain outcomes remain reserved. Numeric credit caps are independent of wallet signer expiry. Credits have one mode ([decision 0042](../../docs/decisions/0042-credits-have-one-mode.md)): a balance comes from x402 settlement or from `grant-credits.ts`, never from `credit-fixture.ts`, which refuses to run outside `bun test`.
- `service-tasks.ts`, `tasks.ts`, `monitoring-runner.ts` — customer tool charges use the credit store. Upstream provider costs remain platform expenses. Credits never authorize a merchant payment, token transfer or trade.

- `session.ts` — `spend()` is the wallet payment choke point for external merchants: price and authorize, reserve the run budget, replenish if needed, then recheck policy and draw the pocket under one lock before paying and writing the receipt. Nothing pays around it. Only after the parent Hedera payment is allowed and reserved, `replenish()` converts the person's USDC to HBAR when the pocket is short: a nested spend keyed `convert:<parent key>`, judged and receipted like any other, never a tool (`docs/decisions/0013`).
- `session.ts` — `spendTrade()` is the typed trading branch of the same authority boundary. It serializes immutable human/rule approval, reserves token principal and native fees in the durable trading book, and persists signing identity before broadcast. Trading capital never uses an invented USD quote; Froggy service charges use platform credits; external merchant payments continue through `spend()` (decision `0021`).
- `conversion.ts` — durable recovery of the USDC transfer and Hedera funding. Persist transaction identifiers before broadcast, reconcile unknown outcomes, and credit the pocket once only after funding is confirmed. The USDC leg is an EIP-3009 authorization the treasury wallet settles (`services.evmRelayFor`), so the person's wallet needs no ETH; without a treasury wallet it is a plain transfer (`docs/decisions/0026`).
...
- `prompt.ts` — what the model reads first: the date and time where the person is, the room the reply lands in (web, Telegram, a paid browse, an unattended run), and the one prompt the web chat, Telegram and paid browse share; the jobs bring their own. Telegram's copy of an alert, report, notice or approval card is filed as an assistant message marked `[Froggy alert]`, `[Froggy report]`, `[Froggy notice]` or `[Froggy card]` (`telegram/pager.ts`), and the prompt names those markers so the model does not mistake a system post for its own words.
