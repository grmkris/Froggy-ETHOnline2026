...
- `packages/wallet/src/credit-store.ts` — the platform credit boundary: one account lock atomically creates a task and reserves its quoted units, then captures successful work or releases failed/cancelled work with the task result. Uncertain outcomes remain reserved. Numeric credit caps are independent of wallet signer expiry. Real and simulated funding cannot share an account.
