...
describe("browser credit allowances", () => {
  ...
  it("retires the task signing door for OAuth callers before wallet authorization", async () => {
    const refused = await handleWalletPay(
      deps,
      new Request("http://localhost:3000/api/wallet/pay", { method: "POST" }),
      workspace(),
      {
        agentTokenId: null,
    ...
    expect(await services.ledger.since(ALICE, 0)).toEqual([]);
  });
});
