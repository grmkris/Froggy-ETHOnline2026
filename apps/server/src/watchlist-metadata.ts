...
const offerData = (value: typeof RecordValue.Type): typeof RecordValue.Type => {
  const { offers } = value;
  const offer = Schema.decodeUnknownResult(RecordValue)(
    Schema.is(Schema.Array(Schema.Json))(offers) && offers.length === 1
      ? offers[0]
      : offers
  );
  return offer._tag === "Success" ? offer.success : value;
};
...
export const parseLinkedData = (
  ...
  if (Schema.is(Schema.Array(Schema.Json))(json)) {
  ...
  return {
    ...
    observation: {
      ...
      price: Schema.is(Schema.Finite.check(Schema.isGreaterThanOrEqualTo(0)))(
        ...
        : null,
      currency:
        currency !== null && /^[A-Z]{3}$/u.test(currency) ? currency : null,
      basis: facts
