...
import type { CardCheckoutId } from "@froggy/domain";
  ...
  private async completeRun(status: HostedRunStatus): Promise<void> {
    ...
    if (!this.state.settled) {
      ...
      this.state = {
        ...
        text:
          this.state.checkoutId !== undefined
            ? "Purchase run finished. See the purchase review for its observed outcome."
    ...
    if (this.state.checkoutId !== undefined) {
      if (this.state.checkoutStage === "inspect") {
        await this.deps.services.cards.inspect(
          { session: this.workspace.session, connectionId: null },
          this.state.checkoutId,
          summary.result ?? "{}"
        );
      } else {
        await this.deps.services.cards
          .outcome(
            this.workspace.userId,
            this.state.checkoutId,
            summary.result ?? "{}"
