...
import type { TokenInspectResult } from "@froggy/protocol";
    ...
    book.set(itemId, {
      ...
      latest:
        data.latest !== null && data.latest.at > observation.at
          ...
          : observation,
      observations,
      snapshotTaskId: snapshotTaskId ?? data.snapshotTaskId,
    });
  });
};
...
const marketResult = (task: Task, network: string, after: number) => {
  if (task.kind !== "service" || task.status !== "done") { return null; }
  const result = serviceTicket(task).data;
  if ((result?.operation !== "token_snapshot" && result?.operation !== "token_inspect" && result?.operation !== "market_search") || result.network !== network || after >= result.observedAt) { return null; }
  return result;
};
...
export const ingestItemTasks = async (
  ...
  await Promise.all(
    tasks.map(async (task) => {
      const result = marketResult(task, source.network, existing?.latest?.at ?? -1);
      if (!result) { return; }
  ...
  return Response.json(
    {
      ...
      item,
      data,
      snapshot: snapshot?.operation === "token_snapshot" ? snapshot : null,
    },
    { headers: { "cache-control": "no-store" } }
  );
};
