...
export const emailItemFacts = (
  ...
  return {
    input: {
      title:
        /code|password|verification|reference|\bpnr\b|confirmation|reservation number/iu.test(
          message.subject
        )
          ? "Saved email details"
          : message.subject.slice(0, 120) || "Saved email details",
      notes: facts
        .map((fact) => fact.value)
        .join("\n")
        .slice(0, 1000),
...
export const handleWatchlistEmail = async (
  ...
  if (!services.email) {
    return Response.json(
      { v: 1, error: "Email is unavailable." },
      { status: 503 }
    );
  }
