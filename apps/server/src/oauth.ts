...
const decodeConsent = Schema.decodeUnknownResult(ConsentBody);
...
const handleConsent = async (
  ...
  const decoded = decodeConsent(await request.json().catch(() => null));
  if (decoded._tag === "Failure") {
    return apiJson(
  ...
  const { decision, granted, params } = decoded.success;
...
const exchangeCode = async (
  ...
  if (
    ...
    return {
      description: `This server issues tokens for ${resourceOf(deps.appOrigin)} only.`,
