...
import { TaskOutcome, WatchlistCapture } from "@froggy/protocol";
...
const priceFor = (deps: TaskDeps, item: WatchlistItem): number | null => {
  ...
  return card &&
    ...
    card.networks?.includes(item.source.network)
    ? card.priceUsdMicros
    : null;
};
const update = async (
...
export const enrichSavedItems = async (
  ...
  await Promise.all(
    entries.map(async (entry) => {
      ...
      if (!intent || !["queued", "running"].includes(intent.status)) {
        return;
      }
      ...
      if (item.source._tag === "wallet") {
        await update(deps, owner, item.id, intent.key, {
          status: "done",
          note: "Address saved. Open it to see activity or add an alert.",
        });
        return;
      }
      if (item.source._tag !== "token") {
      ...
      try {
        ...
        if (item.source._tag === "token") {
          ...
          await update(deps, owner, item.id, intent.key, { taskId: ticket.id });
        } else {
...
export const handleWatchlistCapture = async (
  ...
  const preview = input.previewRef ? await watchlistPreviewFor(owner, input.previewRef) : null;
  const matches = preview?.candidates.some((candidate) => "url" in candidate.source && "url" in item.source && candidate.source.url === item.source.url) === true;
  if (matches && preview?.observation) { await recordItemObservation(deps.services.store, owner, item.id, preview.observation); }
  if (matches && preview?.imageUrl) { await deps.services.store.watchlistData.transact(owner, (book) => { const data = book.get(item.id) ?? emptyWatchlistData(item.id); book.set(item.id, { ...data, imageUrl: preview.imageUrl }); }); }
