...
import type { OAuthTokenRow } from "@froggy/wallet";
...
describe("the token endpoint", () => {
  ...
  it("revokes tokens issued after a concurrent code replay", async () => {
    const base = memoryStore();
    const inserting = Promise.withResolvers<boolean>();
    ...
    const store = {
      ...
      oauth: {
        ...
        tokens: {
          ...base.oauth.tokens,
          insert: async (row: OAuthTokenRow) => {
            if (row.kind === "access") {
              inserting.resolve(true);
              await release.promise;
            }
            await base.oauth.tokens.insert(row);
          },
        },
      },
    };
    const deps = depsAt(NOW, store);
    const { clientId, code, verifier } = await codeFor(deps);
    const fields = exchangeFields(clientId, code, verifier);
    const first = exchange(deps, fields);
    await inserting.promise;
    try {
      expect(await exchangeError(deps, fields)).toBe("invalid_grant");
    } finally {
      release.resolve(true);
    }
    const response = await first;
    const issued = tokens(await response.json());
    expect(
      await resolveAccessToken(store, issued.access_token, NOW)
    ).toBeNull();
    expect(
  ...
  it("refuses a refresh for another resource without consuming the token", async () => {
    const deps = depsAt(NOW);
    ...
    const refreshed = await exchange(deps, {
      ...fields,
      resource: `${ORIGIN}/mcp`,
    });
