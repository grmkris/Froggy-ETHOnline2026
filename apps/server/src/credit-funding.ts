...
import type { CreditState } from "@froggy/protocol";
...
export class CreditFunding {
  ...
  async create(
    ...
    if (isEvmNetwork(input.network)) {
      ...
      const units = String(
        Math.ceil((input.amountUsdMicros * 100_000_000) / rate.usdMicrosPerHbar)
      );
      ...
      // Opening a receiving account does not move the user's USDC or grant an agent authority.
      if (!method.stubbed) {
