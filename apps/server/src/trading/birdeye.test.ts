...
it("snapshot windows fail independently and request real unpadded historical ranges", async () => {
  const { adapter, requests } = fixture([
    overview({ address: ADDRESS, name: "Fixture token", price: 2 }),
    Response.json({
      success: true,
      data: {
        items: [
  ...
  expect(result.token?.priceUsd).toBe(2);
  ...
  expect(requests[1]?.url.pathname).toBe("/defi/v3/ohlcv");
  expect(requests[1]?.url.searchParams.get("padding")).toBe("false");
  expect(requests[1]?.url.searchParams.get("mode")).toBe("range");
  expect(requests[2]?.url.searchParams.get("type")).toBe("1H");
  expect(
    requests.every((request) => request.headers.get("x-chain") === "base")
  ).toBe(true);
});
