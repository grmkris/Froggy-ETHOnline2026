    ...
    Effect.gen(function* layer() {
      ...
      const scheduleTick = setInterval(() => {
        detached("schedule tick", async () => {
          ...
          for (const owner of await services.store.watchlistData.owners()) {
      ...
      const tradeTick = setInterval(() => {
        detached("trade recovery", async () => {
          ...
          for (const owner of await services.store.cards.pendingOwners()) {
            for (const checkout of await services.cards.list(owner)) {
              if (
                checkout.bridge !== null &&
                !checkout.bridge.destinationConfirmed
              ) {
                await services.cards.refresh(owner, checkout.id);
              }
            }
          }
        });
