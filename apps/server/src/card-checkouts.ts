...
const stoppedStage = (checkout: CardCheckout): CardCheckout["stage"] => {
  if (checkout.stage === "order_observed") {
    return "order_observed";
  }
  return checkout.paymentDispatchedAt === null ? "stopped" : "outcome_unknown";
};
const activeMethod = (
  book: CardBook,
  id: PaymentMethodId,
...
export class CardCheckouts {
  ...
  async forget(owner: UserId): Promise<void> {
    const methods = await this.options.store.transact(owner, (book) => [
      ...book.methods.values(),
    ]);
    await Promise.all(
      methods
        ...
        .map(async (method) => {
          await this.revoke(owner, method.id);
        })
    );
  }
  async pending(owner: UserId): Promise<readonly CardCheckoutId[]> {
    return await this.options.store.transact(owner, (book) =>
  ...
  async inspect(
    ...
    try {
      if (summary.length > 16_384) {
        throw new Error("card.summary: checkout summary exceeds the limit.");
      }
  ...
  private async unconfirmedFunding(
    owner: UserId,
    checkout: CardCheckout
  ): Promise<CardCheckout> {
    return await this.options.store.transact(owner, (book) => {
      const current = checkoutIn(book, checkout.id);
      if (current.revision !== checkout.revision) {
        return current;
      }
      if (current.stoppedAt !== null || current.paymentDispatchedAt !== null) {
        return update(book, current, {}, this.options.now());
      }
      const expired = this.options.now() > (current.bridge?.fillDeadline ?? 0);
      return update(
        book,
        current,
        {
          stage: expired ? "recovery_required" : "funding",
          error: expired
            ? "The bridge deadline passed without verified arrival. Recovery is required; no refund or resubmission is implied."
            : "Funding confirmation is unavailable. Credential release is paused while the existing transfer is reconciled.",
        },
        this.options.now()
      );
    });
  ...
  async refresh(owner: UserId, id: CardCheckoutId): Promise<CardCheckout> {
    ...
    if (step === undefined || step.transactionId === null) {
      return await this.unconfirmedFunding(owner, checkout);
    }
  ...
  async reconcile(owner: UserId, id: CardCheckoutId): Promise<CardCheckout> {
    // The owner reviewed their issuer dashboard; this is not an issuer API receipt.
    await this.stop(owner, id);
    return await this.options.store.transact(owner, (book) => {
      const checkout = checkoutIn(book, id);
      return update(
        book,
        checkout,
        {
          reconciledAt: checkout.reconciledAt ?? this.options.now(),
          error:
            "Owner reviewed the issuer dashboard and released the purchase reservation. Card charge remains independently unverified.",
        },
        this.options.now()
      );
    });
  ...
  async dispatchCredentials(
    ...
    const envelope = await this.options.store.transact(owner, (book) => {
      ...
      if (balance !== null) {
        ...
        if (
          this.options.now() - balance.observedAt > 60_000 ||
          balance.observedAt > this.options.now() ||
          BigInt(balance.units) - reserved <
            BigInt(current.funding?.required ?? "0")
        ) {
          throw new Error(
            "card.balance: confirmed Linea funding is no longer sufficient. Review the existing transfer before payment."
          );
        }
      }
    ...
    return await this.options.store.transact(owner, (book) => {
      ...
      return update(
        ...
        checkout,
        {
          stage,
          order: observed
            ? `Confirmation observed on ${checkout.inspection?.merchant ?? "the approved merchant"}`
            : null,
          error: observed
            ? null
