...
- `froggy_credits` takes no arguments and returns available and reserved credits plus the owner's limits.
...
- `froggy_service_run` takes `v: 2`, `service`, `prompt` and a stable `idempotencyKey`; it buys and starts the requested service.
...
`froggy_service_run` arguments have `v: 2`, `service`, `prompt`, and a stable `idempotencyKey`. Status takes `id`. Disconnecting the agent in Froggy revokes every transport.
