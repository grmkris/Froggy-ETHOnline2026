...
The Trading rules panel is in **Services → Trading desk**. Choose a route, exact assets, principal and native-fee caps, maximum entries, maximum open positions, slippage and expiry. Optional research requirements fail closed before each entry; see [decision 0023](decisions/0023-research-gated-rules.md). The Pons route offers template match, launch-window insiders and a top-holder share cap. Uniswap routes on Base, Ethereum and Robinhood offer the top-holder share cap alone: holders are reconstructed from Transfer history on the configured RPC and the rule refuses when that history is incomplete, so young tokens are the realistic audience and long-lived mainnet tokens will usually refuse. Automatic exits are never gated by research. Only the authenticated human can authorize or revoke a rule. An agent may call `froggy_trade_execute` with `tradeId` and an existing `ruleId`; HTTP uses `POST /api/trades/<id>/execute` with `{ "v": 1, "ruleId": "…" }`. Preparation and simulation do not create authority.
