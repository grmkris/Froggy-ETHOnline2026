...
| 11 Sep 2026 15:20 UTC | all twelve, read by pinned ipfs hash (`/api/deployments/id/Qm…`) rather than by subgraph id | **12/12 fresh.** Ethereum at block 25954775; cheapest USDC borrow Euler 2.762%. Every answer's `_meta.deployment` matched the pinned hash. |

**Pinned by hash since 11 Sep 2026.** Until then the registry's entries were subgraph ids, which resolve to whatever version is current, so the receipt's claim of an exact indexed artefact was looser than it read. Each entry now carries the `Qm…` hash of the version being read, queries go to the gateway by that hash on both the Studio and the x402 paths, and an answer whose `_meta.deployment` names another hash is refused as unavailable. The hashes were resolved from each subgraph's current version in the Graph Network subgraph and all twelve answered by hash at their head block that day; the receipt shows the hash beside the block.
