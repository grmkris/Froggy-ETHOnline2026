...
Source commit `a8834a6ee45c0d51a1024867f0af30e7b648235a` was deployed to [the existing public app](https://app-production-58dd.up.railway.app) as Railway deployment `22d613a9-e8b4-407f-b5ba-9c6d2b29372f`. Railway reports success; `/health` returns 200 with live adapters and a usable HBAR rate. The inspected startup logs contained no errors.

At 02:41 UTC, eight runtime source hashes matched the reviewed release. Migration 0025 was applied with SHA-256 `a7e290b741c6adab6e6bb51a711351ce8f8a47a0c2d1d2d10bfdefc678a189a3`. Credit accounts, purchases, charges and ledger entries were initially empty. Historical tasks, the paid paused browser task, legacy sales and both uncertain wallet holds were unchanged. [Release evidence](platform-credits/production-release.json) records the source and migration hashes.

The public browser/API smoke check passed 12/12. Fresh desktop and mobile sessions rendered the landing page and visible email/Google login controls, with no page or console errors. Discovery advertises authenticated credits on both live rails; retired anonymous payment routes return 410 and unauthenticated credit endpoints return 401. Root-document requests aborted during the Privy navigation lifecycle; no API or asset requests failed. See the [public check report](platform-credits/production-report.json), [desktop login](platform-credits/production-desktop-login.png) and [mobile login](platform-credits/production-mobile-login.png).

Real owner funding remains unverified. No mainnet payment was signed or funds spent by these checks. The requested final owner test is a custom $1 USDC purchase, a custom $1 HBAR purchase, then two 1-credit tools. After both purchases and deliveries, the wallet should show its starting balance plus 198 credits, two confirmed purchases and two captured charges. The approved total funding/provider/network budget is $3. This step requires a signed-in owner session, which the available tools do not have.

## Production repair, 13 September 2026 (decision 0042)

Found at 13:17 UTC: the owner's account (`did:privy:cmtpfv5uq01i40cjt3h0ztuvy`) held 2,000 credits from a `test-fixture` purchase (`01a09a66-7bd4-7779-b169-b6056684966f`, `hedera:testnet`, transaction `test-fixture:ctp_01m2d6cyymexwv2tdp0nk895kf`, written at 10:53 UTC by the test-only `fundTestCredits` helper) and was latched `stubbed = true`, so the live browse task `01a09aea-ccff-7bbe-beee-c6d0e3c1e49a` was refused with `credit_mode_mismatch`.

After migration `0032_credits_one_mode` deployed (commit `993d759`, live in the `63d7292` build at 14:05 UTC), one transaction inside the app container deleted the fixture purchase and its funding entry `01a09a66-8042-7779-b169-be48f273e44f` and subtracted its 20,000,000 units (balance 0). `grant-credits.ts` then granted 2,000 credits with the note "Granted by the operator, 13 Sep 2026": available 2,000, held 0. The raised limits ($20 per task, $50 per day), the three expired USDC quotes and the refused charge were left as history. The owner's retry of the browse task is the remaining check.
