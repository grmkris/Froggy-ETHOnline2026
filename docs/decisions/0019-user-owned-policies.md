...
**Status at HEAD, 12 September 2026.** Both remaining gaps are closed. The judgement passed to `authorize` now carries `allowance: this.agentPolicy?.allowance ?? null`, so the action-kind table binds in-process: `transfer` always asks, `askOverUsdMicros` decides even when `SPENDING_LIMITS` is off and the mandate has no `approval_threshold`, and a person with no allowance still runs the threshold-only regime. A commit from the policy editor reaches Privy, the store, and the live session: `Workspaces.existing` looks the person up, `applyAllowance` rebuilds the mandate, and the wallet state is republished so the new numbers show without a reload.

**Conversion.** Nested USDC-to-HBAR conversion is still "only ever nested inside a payment already allowed." The nested spend is judged with `approved: true`, so the ask half cannot fire on its own; the ceiling half may still bind. A parent under the person's per-spend cap is not refused or paused by a conversion whose amount is the parent's plus the floor/multiple the code already computes. A parent over the cap is refused before any conversion starts.
