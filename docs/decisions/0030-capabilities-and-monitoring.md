...
Browser tasks requiring Froggy workspace tools (delegated connections, email and monitoring) remain on the tool-enabled Froggy loop. The concurrent hosted-executor work can handle other browsing, but must not silently omit a required email or workspace capability after purchase.
