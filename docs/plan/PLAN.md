...
| 2.4 | **Privy wallets, policy, signer.** `P_agent` from constants (rules a, a′, b, c) owned by a policy-owner key; `privyAgentSigner` implementing `EvmSigner`; `SignerRefusal` decoded; `wallet_send` provenance; a real raw denial with policy id. | denial card with policy id on the deploy; fixtures | partial: client-minted wallet and the agent as signer under `docs/privy-agent-policy.json` `2f5891c`; typed-data signing under the agent key with the refusal verbatim on the receipt `a438252`. Rules (b) and (c), the policy-owner key and `wallet_send` reaching Privy for a raw denial are open |
...
| 2.9 | **Top-up under rule b.** `signTransaction` plus minimal JSON-RPC, `wallet_topup`, treasury credits the pocket at a documented rate, `mockBridge: true` on the receipt; third top-up denied by aggregation. | Basescan transfer; aggregation result in PRIVY.md | open; **the Privy headline, built before 2.5 is verified live** (planning session); credits the per-user pocket balance |
| 2.10 | **Freeze end to end.** Policy rules wiped with the owner key, signer removed, pocket deleted to treasury, worker frozen. Unfreeze is human-only and rebuilds. | mirror node shows the pocket deleted; no signer | partial `362d920`: flag, abort, approvals denied, browser frozen, signer revoked, unfreeze re-grants. Zeroing the pocket balance and the policy-owner key open; "pocket deleted" becomes "balance zeroed" |
...
Skeletons landed `79a38d7` in `docs/evidence/` (HEDERA, GRAPH, PRIVY, AI-USE) with `TODO(tx)` marks for the on-camera beats; README updated with the workspace picture and surfaces. Remaining: fill each mark the hour its beat lands; the README's demo beats with timestamps, on-chain evidence table and honesty box; a history secret scan script for the Sat 12 flip; the video (owner decided Thu; script Thu from the demo beats, rehearse Thu evening, record Fri: 2 to 4 minutes, 720p or better, human voice, product on screen at 0:00, explorer pages in the back half). Submit Sat 12 by 20:00 CEST; select Privy, The Graph, Hedera.
