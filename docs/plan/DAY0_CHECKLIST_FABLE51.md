...
| No undisclosed prior-project references in code or docs | `grep -rn -iE 'ported from\|lifted from\|project-h\|project-i\|project-b' --include='*.ts' --include='*.tsx' --include='*.md' --include='*.example' . \| grep -v research_FABLE51 \| grep -v _FABLE51.md` | Every hit is either inside the README or AI-USE.md disclosure section with a public link, or the file is rewritten from the protocol docs and the header removed |
