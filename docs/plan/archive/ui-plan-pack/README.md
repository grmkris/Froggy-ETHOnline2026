...
- UI sources under `apps/web` and `packages/ui` have no differences between `d735e67` and this baseline. The earlier controlled desktop/mobile captures therefore remain useful UI evidence. The mobile wallet was above the initial viewport; Connect opened Policy; a failed token creation showed no error. See [the earlier review](../../IMPROVEMENT_REVIEW_2026-09-06.md).
