...
- Open in this lane: the live run of `wallet_send` and `wallet_topup` from a signed-in wallet (the app's one user wallet carries no agent signer yet; owner check), the daily model budget (1.6), the second half of 2.6, the evidence fills as beats land.
...
- **Session A** (the planning session, `session_011fhisYdtVt2vh8DtW4MkPU`): done above: 2.4 remainder, 2.9 top-up with the pocket balance, 2.10 remainder. Next: 1.6 daily budget, then 2.6 second half (payment state machine, unlock token, service card), evidence `TODO(tx)` fills as beats land, the history secret scan script.
