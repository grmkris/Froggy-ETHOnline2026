...
| 2.6 | **Landed 7 Sep (lane 5, ADR 0012): the OAuth 2.1 authorization server, `/mcp`, the consent and manual pages, `froggy login`, the secret-free skill; the public `/agents` page is lane 2's and the live Hermes check is still owed.** Agent onboarding, the official way (Kristjan, 7 Sep 00:20 CEST: "agent onboarding instructions… and official MCP OAuth flow"). A public page `/agents` that a person or an agent can read: what Froggy sells, the three ways in (MCP with OAuth, the skill plus CLI, the raw x402 task API), prices, and the spending rules the person sets. Froggy as a remote MCP server at `/mcp` with the OAuth 2.1 flow MCP clients expect: `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`, dynamic client registration, authorization code with PKCE where the person signs in with Privy and picks the workspace and scopes (one scope per mandate verb: brief, browse, pay), short-lived access tokens bound to that workspace, refresh tokens revocable from the Agents tab beside the legacy `fgy_` tokens. Hermes, Claude, Cursor connect by URL and sign in; no token pasting. Another session has a bearer-token `/api/mcp` in progress (uncommitted, `apps/server/src/mcp.ts`): build the OAuth layer in front of that endpoint, do not duplicate it | run, after coordination with that session | 1.3, 1.7 | An MCP client connects to the live URL through the OAuth flow and buys one brief; the `/agents` page reads well to a stranger; `docs/evidence/HERMES.md` | 6 h, medium |
...
| C | **Done 22:20 CEST.** Froggy's own bot `@froggy_onchainbot`: token and username from the owner's secrets file to Railway by the CLI's stdin, a fresh webhook secret, the webhook registered; `/health` says telegram live and an unsigned webhook call gets 401. Pairing, digest and approval cards are exercised by the owner tomorrow | done |
| D | **Done 00:11 CEST, 7 Sep.** Float `0.0.10847552` (one HBAR from the owner's HashPack created it; 20-HBAR bridge slices from his ETH and his 500 HBAR filled it), service `0.0.10847556`, Blocky402 mainnet settlement `0.0.10571514@1788732364.891462934` on the production path, HCS topic `0.0.10847557`, production flipped by Railway variables (`docs/evidence/HEDERA.md`) | done |
...
| G | **Done 22:30 CEST, proven on testnet** (`docs/evidence/PRIVY.md`, ADR 0009). A cosmos-type Privy wallet per person, created in their name under `PRIVY_HEDERA_POLICY_ID`, opened by the float paying its alias, signing through `raw_sign`; sealed keys stay as the fallback custody and for accounts opened before | nothing |
