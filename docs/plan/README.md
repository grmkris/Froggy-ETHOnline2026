...
12. **`PRD_MCP_SELLER_FABLE51.md`** - written 9 Sep for the Hedera AI & Agentic Payments track: the second door, a small server any outside agent can install, which answers what Froggy sells, buys one for the caller from the caller's own Hedera account, and shows the settlement was real. Three views, what it never does, what it changes in the app, and four open questions.
13. **`../decisions/0020-agent-door.md`** and **`../evidence/AGENT_DOOR_FABLE51.md`** - the door as built on 10 Sep: why it is a second door rather than a widening of the person's `/mcp`, why there is no fallback facilitator and no stub sale, and what was and was not verified. The README section it needs is proposed in **`../README_AGENT_DOOR_FABLE51.md`**.
14. **`RAIL_DECISION_FABLE51.md`** - written 11 Sep for the "drop Hedera, move to Arc or Base?" question: the Hedera footprint at HEAD (about 2,600 lines, 409 tests, four ADRs; removal is days, not an hour), the live prize rules and the changed Arc bounties, Arc verified against Circle's docs (USDC as gas, EIP-3009, Gateway as the only Arc facilitator, Privy support), the four options costed, and the recommendation: keep the rail for Sunday and change the copy; choose the EVM-only rail after the submission.
