...
| new | **Open for Monday: who custodies the person's Hedera key.** Spike 1.8 showed Privy's `raw_sign` refuses Ethereum and Solana wallets but signs for a cosmos-type wallet, which became Hedera account `0.0.10396162` and paid a transfer signed only through Privy; a raw-bytes rule takes no conditions, so Privy cannot cap that leg. Today (task 1.5, live) Froggy opens the account and seals the key under `HEDERA_KEK`; the alternative is a cosmos-type Privy wallet per person, about three hours. | decided tonight: Privy custody (row below) |
| new | **Mainnet everywhere, tonight (Sun 6 Sep 21:40 CEST). Done 00:11 CEST on 7 Sep** after the gate passed (`docs/evidence/HEDERA.md`). Production flips to Hedera mainnet and Base mainnet as soon as the owner's HBAR and USDC land and one Blocky402 mainnet settlement passes; the run flips on its own and posts in the group. Test spend capped at 25 HBAR from the float and $5 USDC from the treasury, every id recorded. | Kristjan |
| new | **No free money on mainnet.** `POCKET_STARTING_USD=0`; a person's Hedera account opens at their first top-up and is funded with that top-up's value in HBAR; the creation fee is Froggy's. Team and demo DIDs get a $1 starting credit. | Kristjan |
| new | **Custody moves to Privy.** Each person's Hedera key is a cosmos-type Privy wallet signing through `raw_sign` under the agent key (spike 1.8); the sealed keys of task 1.5 are replaced. Decided tonight, not Monday. | Kristjan |
| new | **One treasury on Base mainnet**: Privy app wallet `yikihk1kul6vh518z6pers1f` (`0x8Cc232c9EB25b4b20ee448106858e3B6281708C2`) under policy `wdct7xe9re788wr3htum96pw`, which allows only the Graph x402 typed-data payment; it receives top-ups and pays The Graph. | Kristjan |
| new | **The wallet is the first screen.** After login: a dollar total, USDC on Base with the address and Copy, HBAR in the person's Hedera account with its id and Copy, the service credit, Add funds, Top up credit, Connect an agent, recent receipts; the chat below; balances read live from Base and the mirror node. | Kristjan ("does not look like a wallet") |
| new | **Froggy's own Telegram bot**, created by the owner in BotFather; the token reaches Railway from the build box, never through chat. Kermes stays Hermes' bot. | Kristjan |
| new | **Mainnet Hedera accounts by alias**: the run generated the float and service ECDSA keys on the build box (`~/.config/froggy-mainnet.env`, mode 600); the owner funds their EVM aliases `0x9991a9af8c2a9ebebdff49114fb0db90ff9aebe3` (float, ~500 HBAR) and `0xb8aa95d3789db95e0d7661583e9d7a363766f30e` (service, ~2 HBAR), which creates the accounts. | Kristjan |

## Iteration 3, decided in the 7 September team-sync session

The owner's answers in Claude session `ae4fd45e-aae3-4028-ad26-d03dc4593d34` supersede earlier rows where they differ. [ITERATION_3.md](ITERATION_3.md) preserves the decisions and current status: hide spending controls while keeping enforcement, show one USDC-plus-HBAR dollar total, convert USDC when a Hedera payment needs it, support two-way Telegram and schedules, use remote MCP OAuth plus CLI login, provide five workspace pages, redesign services, activate reviewed suppliers, and implement card checkout only after the preceding lanes pass. The name remains Froggy unless the team chooses another; the Monad fitness idea is a separate project.
