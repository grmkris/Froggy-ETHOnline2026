...
export interface RateSource {
  ...
  /** Fetch a fresh rate. Returns false when it could not. */
  ...
  return {
    ...
    refresh: async () => {
      try {
        ...
        const { current_rate: current } = decoded.success;
        if (current.hbar_equivalent === 0) {
          return false;
        }
        rate = {
          expiresAt: current.expiration_time,
          usdMicrosPerHbar:
            (current.cent_equivalent * MICROS_PER_CENT) /
            current.hbar_equivalent,
        };
        return true;
      } catch {
