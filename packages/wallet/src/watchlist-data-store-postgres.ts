  ...
  return {
    transact: async (owner, operation) =>
      await db.transaction(async (tx) => {
        ...
        const changed = [...book].filter(
          ([itemId, document]) =>
            !Bun.deepEquals(before.get(itemId), document, true)
        );
        if (changed.length > 0) {
          await tx
            .insert(savedItemData)
            .values(
              changed.map(([itemId, document]) => ({
                userId: owner,
                itemId,
                document,
              }))
            ...
            .onConflictDoUpdate({
              target: [savedItemData.userId, savedItemData.itemId],
    ...
    owners: async () => {
      const rows = await db
        .selectDistinct({ id: savedItemData.userId })
        .from(savedItemData);
