  ...
  return {
        ...
        await Promise.all(
          [...after.methods.values()].map(async (method) => {
            ...
            await (envelope === undefined
              ? tx
                  .delete(paymentMethodCredentials)
                  .where(eq(paymentMethodCredentials.id, method.id))
              : tx
                  .insert(paymentMethodCredentials)
                  .values({ id: method.id, userId: owner, envelope })
                  .onConflictDoUpdate({
                    target: paymentMethodCredentials.id,
                    set: { envelope },
                    setWhere: eq(paymentMethodCredentials.userId, owner),
                  }));
          })
        );
        await Promise.all(
    ...
    pendingOwners: async () => {
      const rows = await db
        ...
        .from(cardCheckouts)
        .where(
          raw`(${cardCheckouts.document}->'bridge'->>'destinationConfirmed' = 'false' OR ${cardCheckouts.document}->'bridge'->>'sourceConfirmed' = 'false')`
        );
