...
for (const width of [1440, 390, 320]) {
  test(`the landing is public and responsive at ${width}px`, async ({
    ...
    await page.screenshot({
      path: testInfo.outputPath(`landing-features-${width}.png`),
    });
    await page.locator("#setup").scrollIntoViewIfNeeded();
    await page.screenshot({
      path: testInfo.outputPath(`landing-setup-${width}.png`),
    });
...
test("the promo loads on intent, pauses offscreen and replays", async ({
  page,
}) => {
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.goto("/landing");
  ...
  await page.getByRole("button", { name: "Replay promo", exact: true }).click();
  await expect(
    page.getByRole("button", { name: "Pause promo", exact: true })
  ).toBeVisible();
  expect(
