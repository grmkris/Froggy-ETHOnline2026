...
| **7 · Four-line approval ledger** | **landed** — optional `breakdown` on `ApprovalRequest` (product, known delivery/network or fees, agent spend so far today). Old cards without the field still decode and render as amount-plus-detail. Display only; no decision change. |
| D · Logo wake/reveal + UI feedback | done — `prototype/ui-feedback.html` and a wake control on the motion page, gated by `tools/check-feedback.mjs` (12 assertions) |
