...
| DesignSync, after `/design-login` | `list_projects`, `get_project`, `list_files` | Authorization succeeded. The id in `.design-sync/config.json` returns **404 `project not found`**. Account holds `Industry` (populated, different product) and `Design System` (empty), both `PROJECT_TYPE_DESIGN_SYSTEM`, both editable. | **live-verified** |
| Froggy DS project created and populated | `create_project`, `finalize_plan`, `write_files`, `list_files` | `3f609607-6f91-4b05-9ec7-6dd2bb550092`; 8 files written and read back. | **live-verified** |
