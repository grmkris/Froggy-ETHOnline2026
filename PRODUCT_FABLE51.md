...
**Service side.** payTo is a real 0.0.x account (aliases are rejected), an HCS topic for receipts, facilitator `https://api.testnet.blocky402.com`, network `hedera:testnet`, price 0.0125 tHBAR per protocol (0.05 for the four-protocol brief), a one-time receipt token in the paid response so the shared Chrome can navigate to the unlocked page (the browser never pays; the host pays), a `/.well-known/x402.json` service card.
...
- **Hedera.** We host the x402-gated brief service on `hedera:testnet`, settled through Blocky402, metered per protocol, an HCS message per settlement, a service card, a curl-able 402 printed in the README. Our agent and the daily cron consume it; the agent also pays peer endpoints from the directory.
...
- **Telegram** is a pager with pairing codes; Telegram OAuth login only if a 30-minute test on Saturday resolves the same DID; Privy Intents only inside the Thursday box.
...
Owners: **A** owns the Chrome slice and hosting full-time (Kristjan); **B** owns Privy, the Hedera service, the Graph registry and the digest (the third builder, else Kristjan takes Privy and Jonas takes the Hedera service and Graph registry with a 30-minute daily review); **Jonas** owns build-in-public, landing, consent, the Telegram bot, testers, evidence files and the video. Wallet-slice hours after the cuts below: about 40 across Days 1-5, one person at eight hours a day.

Cuts made now, regardless of headcount: Base Sepolia is rule (b)'s default (the 296 spike is a 30-minute time box whose only effect is swapping the chain); pocket key encrypted under one env key, no KMS story; HCS is a two-hour item after the first settlement, not before; the guest path ships without sweep (prefund once, cap 40, delete rows by hand on Day 8); the public receipts page is a plain HTML page and VALIDATION.md carries the counters; the digest cron moves to Day 6 and is cut first; the deny-suite is fixtures; ERC-8004, Intents, the MCP tool and the watch are out.
...
- **A Messari deployment is stale or the client is still single-subgraph at judging** (a Composable-track disqualifier). The four-deployment registry lands Day 2 before any other Graph work; freshness gate; Spark as spare; never fewer than two deployments; ids and blocks on every receipt.
...
- **Nobody shows up, or the third builder is out.** Peer sellers are individually addressable and need us; the guest path removes signup friction; the honest number goes in VALIDATION.md. If the third builder is out by Sat 10:00, stretch items are cancelled that morning.

---

## 12. Open decisions (recommendation in bold)

- Is the third builder in? **Answer by Sat 10:00; plan for no.**
- Worker-per-user or puppeteer-core contexts for hosted isolation? **puppeteer-core contexts by default, per the hour estimates; the Chrome owner decides after the Saturday measurement.**
